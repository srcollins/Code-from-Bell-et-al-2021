function inds2Plot = sortpDatCenterStimbyStimConds(pDatPDL,cellType,Power,numStim,varargin)
...
%converterd to sortpDatCenterStimbyStimConds on 2021-01-23 to be used for
%centerstim data.
...
if any(boolRegExp(cellType,'all'))
    ...
    for c=1:length(cellType)
        tempCinds=find(arrayfun(@(x) boolRegExp(x.condLabel,cellType{c}),pDatPDL));
...
if any(boolRegExp(Power,'all'))
    ...
    for p=1:length(Power)
        tempPinds=find(arrayfun(@(x) boolRegExp(x.stimLabel{1},Power{p}),pDatPDL));
...
if ~isempty(numStim)
    ...
    inds2Plot=intersect(cellTinds,pwrInds);
