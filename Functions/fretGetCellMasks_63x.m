function [maskFinal,cellCoors]=fretGetCellMasks_63x(imForSeg0, objectRectangles, minSize, imCFP, imFRET,sharpParam)
...
for i=1:length(objectRectangles)
    maskFinal=processSingleObjectRectangle(maskFinal,objectRectangles{i},imForSeg0,minSize,sharpParam);
end
...
%------------------------------------------------------------------------------
function maskFinal=processSingleObjectRectangle(maskFinal,objRect,imForSeg0,minSize,sharpParam)
...
maskThisObj=imdilate(maskThisObj,strel('disk',2));
maskThisObj=imerode(maskThisObj,strel('disk',3)); 
